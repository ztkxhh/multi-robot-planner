...
Beziercurve::Beziercurve(const int &total)
{
    ...
    // this->_x_third.reserve(total + 1);
    // this->_y_third.reserve(total + 1);
...
Dynamics_S Beziercurve::get_dym(double &s)
{
    ...
    dym.x_fir = d.first;
    dym.y_fir = d.second;
    // dym.x_third = ddd.first;
    // dym.y_third = ddd.second;
    return dym;
}
...
void Beziercurve::Atrributes()
{
    ...
    for (int j = 0; j <= this->_total; ++j)
    {
        ...
        // this->_x_third.push_back(current_dym.x_third);
        // this->_y_third.push_back(current_dym.y_third);
